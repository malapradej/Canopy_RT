...
class rt_layers():
  ...
  def __init__(self, Tol = 1.e-6, Iter = 200, K = 20, N = 16,\
    ...
    # direct sunlight reflected by soil eq(19)
    self.Ir = 2. * -self.refl_s * self.mu_s * \
        self.I_f(self.sun0, self.Lc, self.I0)
    # discrete ordinate equations
    g = G(self.views,self.arch) * self.CI
    ...
    self.Gx = g
    self.Gs = G(self.sun0,arch) * self.CI
    ...
    for (i, k) in enumerate(self.mid_ks):
      for (j, v) in enumerate(self.views):
        self.Q1nodes[i,j] = self.Q1(j,k) 
        self.Q2nodes[i,j] = self.Q2(j,k)
  ...
  def I_down(self, k):
    '''The discrete ordinate downward equation. See Myneni eq (31).
    '''
    n = self.n
  ...
  def I_up(self, k):
    '''The discrete ordinate upward equation. See Myneni eq (32).
    '''
    n = self.n
  ...
  def reverse(self):
    '''Reverses the transmissivity at soil boundary. See Myneni 
    eq (42).
    '''
  ...
  def solve(self):
    ...
    for i in range(self.Iter):
      ...
      print self.Inodes[0,0,:self.n]     
      if self.converge():
        #pdb.set_trace()
        I_TOC = - 1. / self.mu_s * self.Inodes[0,0,:self.n] + \
            self.I_f(self.views[:self.n],self.Lc,self.Ir)# eq (46)
        I_soil = (self.Inodes[self.K-1,2,self.n:] / -self.mu_s + \
            self.I_f(self.sun0,self.Lc,self.I0))\
             * (1. - self.refl_s) # eq (48) and (49)
  ...
  def __repr__(self):
    ...
    refl_s = %.3f, I0 = %.4f,
    sun0 = %.3f, arch = %s, CI = %.2f''' % (self.Tol, self.Iter, self.K, \
        self.N, self.Lc, self.refl, self.trans, self.refl_s, \
        self.I0, self.sun0*180./np.pi, self.arch, self.CI)
  ...
  def I_f(self, angle, L, I):
    ...
    of the following:
    I_f = |cos(angle)| * I * exp(G(angle)*L*CI/cos(angle))
    Input: angle - the illumination zenith angle, L - LAI, I - 
    the intial intensity or flux.
    Output: the intensity or flux at L.
    '''
    mu = np.cos(angle)
    i =  I * np.exp(-abs(G(angle,self.arch)*L*self.CI/mu))
    return i
  ...
  def J(self, index_view, Ia):
    ...
    if isinstance(Ia, np.ndarray):
      ...
      #j = self.albedo / 2. * np.sum(integ*(self.gauss_wt/2.))
      j = self.albedo / 2. * np.sum(integ*(self.gauss_wt)) 
        # /2 due to gauss_wt summing to 2 over both hemispheres
    else:
